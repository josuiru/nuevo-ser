...
/// Test de paridad Dart/PHP del motor adaptativo, perfil P2 (detección
/// F1). Consume `test/fixtures/motor_p2.json`. El test PHP equivalente
/// (`wp-plugin/nuevo-ser-core/tests/test_paridad_motor_p2.php`) lee la
/// misma fixture y aplica las mismas comprobaciones — si los dos pasan,
/// hay paridad bit a bit.
void main() {
  group('paridad Dart/PHP — motor P2 (detección F1)', () {
    late List<dynamic> casos;

    setUpAll(() {
      final fixture = File('test/fixtures/motor_p2.json').readAsStringSync();
      casos = (jsonDecode(fixture) as Map<String, dynamic>)['casos'] as List;
    });

    test('todos los casos coinciden con el motor P2 + defaultP2', () {
        ...
        for (final p in (caso['secuencia'] as List).cast<Map<String, dynamic>>()) {
          estado = motor.actualizarMaestria(
            ...
            payload: SessionPayload(
              ...
              senalEsperada: p['senalEsperada'] as bool?,
              clasePredicha: p['clasePredicha'] as bool?,
            ),
            idPerfil: idPerfilP2,
            config: ProfileConfig.defaultP2,
          );
        }
...
EstadoHabilidad _estadoDesdeJson(Map<String, dynamic> json) {
  final intentos = (json['ir'] as List)
      ...
      .map((m) => IntentoHabilidad(
            ...
            senalEsperada: m['se'] as bool?,
            clasePredicha: m['cp'] as bool?,
          ))
      .toList();
...
void _expectarIgualdad(EstadoHabilidad obtenido, EstadoHabilidad esperado, String nombre) {
  ...
  for (var i = 0; i < esperado.intentosRecientes.length; i++) {
    ...
    expect(o.senalEsperada, e.senalEsperada, reason: '[$nombre] ir[$i].se');
    expect(o.clasePredicha, e.clasePredicha, reason: '[$nombre] ir[$i].cp');
  }
}
