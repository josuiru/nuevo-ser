...
/// Test de paridad Dart/PHP del motor adaptativo (doc §6.2).
///
/// Consume `test/fixtures/motor_p1.json`. El test PHP equivalente
/// (`wp-plugin/nuevo-ser-core/tests/test_paridad_motor.php`) lee la
/// misma fixture y aplica las mismas comprobaciones — si los dos
/// pasan, hay paridad bit a bit del motor entre cliente y servidor.
///
/// Cualquier cambio en la fixture o en P1Precision debe verificarse en
/// ambos lados: si solo pasa uno, hay regresión.
void main() {
  group('paridad Dart/PHP — motor P1', () {
    late List<dynamic> casos;

    setUpAll(() {
      final fixture = File('test/fixtures/motor_p1.json').readAsStringSync();
      casos = (jsonDecode(fixture) as Map<String, dynamic>)['casos'] as List;
    });

    test('todos los casos de la fixture coinciden con el resultado del motor', () {
