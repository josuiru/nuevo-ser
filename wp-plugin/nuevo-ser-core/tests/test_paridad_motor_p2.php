<?php
/**
 * Test de paridad PHP/Dart del motor adaptativo, perfil P2 (detección F1).
 *
 * Carga `packages/nuevo_ser_core/test/fixtures/motor_p2.json` — la misma
 * fixture que `mastery_engine_paridad_p2_test.dart` — y verifica que el
 * motor PHP produce idéntico resultado.
 *
 * Ejecutable directo: `php tests/test_paridad_motor_p2.php` desde la raíz
...
$ruta_fixture = __DIR__
	. '/../../../packages/nuevo_ser_core/test/fixtures/motor_p2.json';
...
$config   = NS_Profile_Config::default_p2();
$fallidos = 0;
...
foreach ( $fixture['casos'] as $caso ) {
	...
	foreach ( $caso['secuencia'] as $payload ) {
		$estado = $motor->actualizar_maestria(
			$estado,
			$payload,
			NS_MASTERY_ID_PERFIL_P2,
			$config
		);
	}

	$esperado = $caso['esperado_final'];
	$errores  = comparar_estado_p2( $estado, $esperado );
...
if ( $fallidos > 0 ) {
	fwrite( STDERR, "\n$fallidos caso(s) divergen entre PHP y la fixture P2.\n" );
	exit( 1 );
}

echo "\nOK — paridad PHP/Dart confirmada (P2) sobre " . count( $fixture['casos'] ) . " casos.\n";
exit( 0 );

// ---------------------------------------------------------------------

function comparar_estado_p2( array $obtenido, array $esperado ): array {
	$errores = array();
	...
	for ( $i = 0; $i < count( $esperado['ir'] ); $i++ ) {
		...
		foreach ( array( 'se', 'cp' ) as $campo ) {
			...
			if ( $tiene_e && (bool) $o[ $campo ] !== (bool) $e[ $campo ] ) {
				$errores[] = "ir[$i].$campo: esperado="
					. var_export( $e[ $campo ], true )
					. ' obtenido=' . var_export( $o[ $campo ], true );
			}
