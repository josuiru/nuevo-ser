...
- **Cliente Dart** `ClienteCompanion` con cinco métodos (crear/listar cuaderno, crear/listar mosaicos, unirse aula). Modelos `EntradaCuaderno`/`ListadoEntradasCuaderno`, `Mosaico`/`ListadoMosaicos`, `MembresiaAula`. `unirseAula` normaliza el code a mayúsculas y trim antes de meterlo en el path. 28 tests con `MockClient`. Patrón gemelo de `ClienteApi` (core) y `ClienteTutor` (tutor): sin lógica de negocio, errores tipados con `ExcepcionApi`.
- **Smoke PHP** `tests/test_companion_cuaderno.php` (27 casos), `tests/test_companion_mosaicos.php` (38 casos), `tests/test_companion_aulas.php` (13 casos: códigos válidos, longitudes mín/máx, regex con guion/underscore/Ñ, normalización lowercase y espacios).
- **Pendientes acoplados a auth de profesor** (no decidida): `POST /classrooms` (creación de aula) y `GET /classrooms/{id}/aggregates`. JWT actual sólo lleva `nino_id`; falta acordar entre cookie WP+nonce, Application Passwords o JWT-de-profesor antes de cablear estos dos endpoints.

Plugin WP en v0.7.0. Tests: 325 (uno-roto) + 55 (nuevo_ser_core) + 22 (nuevo_ser_tutor) + 28 (nuevo_ser_companion) Dart + 6 PHP smoke (filtro_tutor, jwt_tutor, paridad_motor, companion_cuaderno, companion_mosaicos, companion_aulas). `flutter analyze` limpio en los 5 paquetes.
