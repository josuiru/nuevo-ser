...
- **`apps/solera-aceitera/`** — almazaras pequeñas y medianas (100-2000 hl/campaña). Cuaderno PAC olivar (RD 1311/2012) + libro de movimientos del aceite (RD 760/2021 + AICA) + DOP olivar + IA visual plagas olivar + cierre fiscal REAGP. **F1-A1 a F1-A6 cerradas** (esqueleto + 14 modelos POJO + sqflite v1 + 14 pantallas con navegación NavigationBar + 2 generadores PDF con sello PROVISIONAL R1 + 5 catálogos curados provisionales con compilador CSV→Dart + banner provisional + autocomplete contra catálogo en alta de parcela y nuevo tratamiento + 46 tests). Branding verde oliva+crema.
