...
- **Estación 2.1 del Arco 2 + latentes 2.A implementadas (F2-1, F2-2)**: `apps/las-versiones/lib/dominio/escenas_arco_2.dart` cubre la apertura 2.0.1, las 6 cinemáticas de la Estación 2.1 "Pompaelo bajo Iruña" (doc 08 §2.1.1–2.1.6) y las 2 cinemáticas latentes post-Estación 2.1 (2.A.1 *El libro de Quintiliano* + 2.A.2 *Marina y los descansos*). Total: 9 cinemáticas en el catálogo del Arco 2. El orquestador encadena Arco 1 → Arco 2 cruzando `arco_1_cerrado_por_la_cronista`; las latentes 2.A.x requieren `arco_2_estacion_1_cerrada` (que la 2.1.6 activa) y se reproducen en orden secuencial (2.A.2 detrás de 2.A.1). La fase jugable de la Brecha 2.1 (declarar 6 afirmaciones con confianza sobre la inscripción) requiere refactor de `FaseBrecha` para admitir distintos números de afirmaciones por Brecha — pendiente, registrado en `BLOQUEOS-PENDIENTES.md`. La inscripción romana mostrada en 2.1.2 es **modelo literario verosímil** basado en formularios honoríficos genéricos, no una inscripción real catalogada. Las latentes 2.A no requieren sustituciones diegéticas (Quintiliano y la edición Cousin son referencias trazables; la frase de Antonio es comentario crítico genérico). Las 25+ cinemáticas restantes + Brechas 2.2/2.3/2.4 + Mosaico M2 quedan pendientes.
