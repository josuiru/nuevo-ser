...
- **`POST /companion/mosaicos`** — `NS_Companion_Mosaicos::crear_mosaico` añade el tercer endpoint real (quedan 6 en 501). Un mosaico es el "trabajo final" de un arco: shape parecido a cuaderno pero con `arc_id` obligatorio (VARCHAR 64), `format` opcional (VARCHAR 32, lowercase con `_`), y `required_anchors`/`fulfilled_anchors` que aceptan **lista u objeto** (PHP `is_array` cubre ambos; `Object?` en Dart). `qualitative_feedback` opcional como string. `completed_at` lo pone el servidor (un mosaico que llega aquí está terminado). Devuelve 201 con `{id, game_id, arc_id, format, title, content_ref, completed_at}` y header `Location`. Sharing con aulas/cuidadores no se acepta todavía, igual que cuaderno.
- **Cliente Dart** `ClienteCompanion.crearEntradaCuaderno`/`listarEntradasCuaderno`/`crearMosaico`. Modelos `EntradaCuaderno`, `ListadoEntradasCuaderno` y `Mosaico` (anchors como `Object?` para soportar ambos shapes; opcionales preservados del cliente al respond). Factory `EntradaCuaderno.desdeJsonListado` parsea todos los campos del servidor. 16 tests con `MockClient`. Patrón gemelo de `ClienteApi` (core) y `ClienteTutor` (tutor): sin lógica de negocio, errores tipados con `ExcepcionApi`.
- **Smoke PHP** `tests/test_companion_cuaderno.php` (13 + 14 casos) y `tests/test_companion_mosaicos.php` (24 casos: campos requeridos, longitudes, regex de `format`, anchors lista/objeto/inválido, qualitative_feedback string-only, frontera 64 chars de arc_id, multi-error).

Plugin WP en v0.7.0. Tests: 325 (uno-roto) + 55 (nuevo_ser_core) + 22 (nuevo_ser_tutor) + 16 (nuevo_ser_companion) Dart + 5 PHP smoke (filtro_tutor, jwt_tutor, paridad_motor, companion_cuaderno, companion_mosaicos). `flutter analyze` limpio en los 5 paquetes.
