...
- **Estación 2.1 del Arco 2 implementada (F2-1)**: `apps/las-versiones/lib/dominio/escenas_arco_2.dart` cubre la apertura 2.0.1 + las 6 cinemáticas de la Estación 2.1 "Pompaelo bajo Iruña" (doc 08 §2.1.1–2.1.6): bajada al sótano del Archivo, descubrimiento de la inscripción romana, Karim enseña convenciones epigráficas, debate sobre el sesgo del productor (HF.09), Concilio narrativo con Begoña/Joana, primer apunte de Pompaelo en el Cuaderno. El orquestador encadena Arco 1 → Arco 2 cruzando `arco_1_cerrado_por_la_cronista` y recorre las 7 cinemáticas por `flagsRequeridos`/`flagDeSalida`. La fase jugable de la Brecha 2.1 (declarar 6 afirmaciones con confianza sobre la inscripción) requiere refactor de `FaseBrecha` para admitir distintos números de afirmaciones por Brecha — pendiente, registrado en `BLOQUEOS-PENDIENTES.md`. La inscripción romana mostrada en 2.1.2 es **modelo literario verosímil** basado en formularios honoríficos genéricos, no una inscripción real catalogada (registro completo de sustituciones diegéticas en BLOQUEOS). Las 27+ cinemáticas restantes + Brechas 2.2/2.3/2.4 + Mosaico M2 quedan pendientes.
