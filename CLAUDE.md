...
- **Cliente Dart** `ClienteCompanion` con cuatro métodos (crear/listar entradas cuaderno + crear/listar mosaicos). Modelos `EntradaCuaderno`/`ListadoEntradasCuaderno`, `Mosaico`/`ListadoMosaicos`. Factories `desdeJsonListado` parsean todos los campos del servidor; en `Mosaico`, `_coleccionOpcional` acepta tanto List como Map para anchors. 20 tests con `MockClient`. Patrón gemelo de `ClienteApi` (core) y `ClienteTutor` (tutor): sin lógica de negocio, errores tipados con `ExcepcionApi`.
- **Smoke PHP** `tests/test_companion_cuaderno.php` (13 + 14 casos) y `tests/test_companion_mosaicos.php` (24 + 14 casos: incluye validación de query del listado con arc_id, frontera 64 chars, fronteras 100/101).

Plugin WP en v0.7.0. Tests: 325 (uno-roto) + 55 (nuevo_ser_core) + 22 (nuevo_ser_tutor) + 20 (nuevo_ser_companion) Dart + 5 PHP smoke (filtro_tutor, jwt_tutor, paridad_motor, companion_cuaderno, companion_mosaicos). `flutter analyze` limpio en los 5 paquetes.
