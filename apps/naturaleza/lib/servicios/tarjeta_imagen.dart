...
Future<File> generarTarjetaHallazgo(Hallazgo hallazgo) async {
  ...
  if (hallazgo.rutaFoto != null) {
    ...
    final iconoCategoria = switch (hallazgo.categoria) {
      'planta' => '🌿',
      'insecto' => '🦋',
      _ => '🐾',
    };
    _texto(canvas, iconoCategoria, cabecera.center.dx, cabecera.center.dy - 60, fontSize: 160, alineacion: TextAlign.center, ancho: 200);
  ...
  final tituloPrincipal = hallazgo.nombreComun.isNotEmpty
      ? hallazgo.nombreComun
      : (hallazgo.especie.isNotEmpty ? hallazgo.especie : 'Hallazgo de naturaleza');
  yActual = _bloque(canvas, tituloPrincipal, yActual, fontSize: 56, peso: FontWeight.bold);
  yActual += 8;
  final subtitulo = [
    if (hallazgo.especie.isNotEmpty && hallazgo.especie != tituloPrincipal) hallazgo.especie,
    if (hallazgo.taxonomia.isNotEmpty) hallazgo.taxonomia,
  ].join('  ·  ');
  ...
  if (hallazgo.habitat.isNotEmpty) {
    yActual = _bloque(canvas, '🌍  ${hallazgo.habitat}', yActual, fontSize: 22, color: Colors.black54);
  }

  _texto(canvas, 'Naturaleza · cuaderno de campo', _padding, _alto - _padding - 24, fontSize: 18, color: const Color(0xFF8A8268));
