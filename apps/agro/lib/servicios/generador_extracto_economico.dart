...
/// Genera el extracto económico anual del agricultor (libro
...
///  7. Detalle cronológico de ingresos.
///  8. Detalle cronológico de gastos.
///
/// **Limitaciones v1 provisional**:
///  - El reparto proporcional por hectárea de gastos imputados a
///    `cultivo_general` entre las parcelas con ese cultivo NO se
///    calcula — los apuntes con esa imputación se listan tal cual
///    con el importe íntegro asignable al cultivo, y la nota final
///    del PDF lo señala. Cuando el asesor fiscal valide el método
///    de reparto, el cálculo se mueve aquí (registrado en
...
Future<File> generarExtractoEconomico({
  ...
  return generarInformePeriodicoPdf(
    ...
    subtituloCabecera:
        'Agricultura · Libro registro provisional · Asesor fiscal pendiente',
...
String _nombreCultivo(String id) {
  if (id.isEmpty) return '—';
  for (final c in catalogoCultivos) {
    if (c.id == id) return c.nombreVisible;
  }
  return id;
}
...
String _etiquetaImputacion(ApunteGasto g) {
  if (g.esFincaConcreta) return 'Parcela';
  if (g.esCultivoGeneral) return _nombreCultivo(g.cultivoId);
  return 'General';
}
