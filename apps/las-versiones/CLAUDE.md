...
251 tests verde, analyzer limpio. Stack: shared_preferences + StatefulWidget + CustomPainter (sin Flame/Isar/Riverpod aún — siguen pospuestos hasta tener razón real, según el principio del README de `nuevo_ser_core`).
...
**Estación 2.1 del Arco 2 + latentes 2.A implementadas (F2-1, F2-2)**. `lib/dominio/escenas_arco_2.dart` cubre la apertura 2.0.1 ("El primer día del arco"), las 6 cinemáticas de la Estación 2.1 "Pompaelo bajo Iruña" (doc 08 §2.1) y las dos cinemáticas latentes post-Estación 2.1: 2.A.1 *El libro de Quintiliano* (Maren pide bibliografía a Antonio antes de Calahorra; estudio del padre, edición Cousin de la *Institutio Oratoria*) y 2.A.2 *Marina y los descansos* (cocina del Archivo, Marina siembra "Calahorra te va a tocar"). Total: 9 cinemáticas. Se encadenan por `flagsRequeridos`/`flagDeSalida`; la 2.A.1 requiere `arco_2_estacion_1_cerrada` y la 2.A.2 requiere `escena_2_a_1_vista` para mantener el orden del doc 08 (padre antes que Marina). Tras cerrar la 2.A.2 el orquestador cae al esqueleto porque la 2.2.1 todavía no está implementada. La fase jugable de la Brecha 2.1 (declarar 6 afirmaciones con confianza sobre la inscripción) **no está implementada** — requiere refactor del modelo `FaseBrecha` para admitir distintos números de afirmaciones por Brecha; la pedagogía se enseña narrativamente en las cinemáticas mientras tanto. Las 25+ cinemáticas restantes + Brechas 2.2/2.3/2.4 + Mosaico M2 quedan pendientes (registro completo en `BLOQUEOS-PENDIENTES.md`).
