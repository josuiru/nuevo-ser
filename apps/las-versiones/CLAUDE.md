...
247 tests verde, analyzer limpio. Stack: shared_preferences + StatefulWidget + CustomPainter (sin Flame/Isar/Riverpod aún — siguen pospuestos hasta tener razón real, según el principio del README de `nuevo_ser_core`).
...
**Estación 2.1 del Arco 2 implementada (F2-1)**. `lib/dominio/escenas_arco_2.dart` cubre la apertura 2.0.1 ("El primer día del arco") + las 6 cinemáticas de la Estación 2.1 "Pompaelo bajo Iruña" (doc 08 §2.1): bajada al sótano, descubrimiento de la inscripción romana, Karim enseña convenciones epigráficas, debate sobre el sesgo del productor (HF.09 — "una inscripción no es un documento neutral. Es propaganda"), Concilio narrativo con Begoña/Joana, primer apunte de Pompaelo en el Cuaderno. Las 7 cinemáticas se encadenan por `flagsRequeridos`/`flagDeSalida`. Tras cerrar la 2.1.6 el orquestador cae al esqueleto porque la 2.A.1 todavía no está implementada. La fase jugable de la Brecha 2.1 (declarar 6 afirmaciones con confianza sobre la inscripción) **no está implementada** — requiere refactor del modelo `FaseBrecha` para admitir distintos números de afirmaciones por Brecha; la pedagogía se enseña narrativamente en las cinemáticas mientras tanto. Las 27+ cinemáticas restantes + Brechas 2.2/2.3/2.4 + Mosaico M2 quedan pendientes (registro completo en `BLOQUEOS-PENDIENTES.md`).
