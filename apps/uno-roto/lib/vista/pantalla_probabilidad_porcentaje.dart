...
class PantallaProbabilidadPorcentaje extends StatefulWidget {
  final ProblemaProbabilidadPorcentaje? problemaPredeterminado;

  const PantallaProbabilidadPorcentaje({
    super.key,
    this.problemaPredeterminado,
  });

  @override
  State<PantallaProbabilidadPorcentaje> createState() =>
      _PantallaProbabilidadPorcentajeState();
}

class _PantallaProbabilidadPorcentajeState
    extends State<PantallaProbabilidadPorcentaje>
    with SingleTickerProviderStateMixin {
  late final AnimationController _controladorCielo;
  late final EstadoPistaPuzzle _pista;
  late ProblemaProbabilidadPorcentaje _problema;
  int? _indiceSeleccionado;
  bool _revelado = false;
  bool _mostrandoDemo = false;
  static const _idDemo = 'probabilidad_porcentaje';
  ...
  void initState() {
    ...
    _problema = widget.problemaPredeterminado ??
        GeneradorProbabilidadPorcentaje().generar(dificultad: 1);
  ...
  void _elegir(int indice) {
    ...
    if (_problema.esCorrecta(indice)) {
      ...
      comprobarYAyudarSiProcede(context, _pista, TipoFragmentoEnTejado.probabilidadPorcentaje);
      if (!mounted) return;
        ...
        builder: (_, __) {
          return Stack(
            ...
            children: [
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          const Spacer(),
                          Text(AppLocalizations.of(contexto).puzzleHeaderPProb,
                            style: const TextStyle(
                      ...
                      Text(
                        AppLocalizations.of(contexto).puzzleInstrPEquals(
                          _problema.probabilidad.numerador,
                          _problema.probabilidad.denominador,
                        ),
                        style: const TextStyle(
                          color: PaletaNeon.textoPrincipal,
                          fontSize: 28,
                      ...
                      Text(
                        AppLocalizations.of(contexto).puzzleInstrComoPorcentaje,
                      ...
                      Expanded(
                        child: GridView.count(
                          ...
                          children: [
                            for (var indice = 0;
                                indice < _problema.candidatosPorcentaje.length;
                                indice++)
                              _TarjetaCandidato(
                                etiqueta:
                                    '${_problema.candidatosPorcentaje[indice]} %',
              ...
              BotonAyudaPuzzle(destacar: _pista.activa, tipo: TipoFragmentoEnTejado.probabilidadPorcentaje),
              if (_mostrandoDemo)
                OverlayDemoPuzzle(
