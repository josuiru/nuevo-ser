...
/// Puzzle PROP.05: el niño ve "12 de 50 → ¿qué porcentaje?" y elige
/// el resultado entre cuatro candidatos. Inverso de PROP.04 (calcular
/// el % de una cantidad): aquí calcula el % a partir de la parte y el
/// total.
class PantallaPorcentajeDe extends StatefulWidget {
  final ProblemaPorcentajeDe? problemaPredeterminado;

  const PantallaPorcentajeDe({super.key, this.problemaPredeterminado});

  @override
  State<PantallaPorcentajeDe> createState() => _PantallaPorcentajeDeState();
}

class _PantallaPorcentajeDeState extends State<PantallaPorcentajeDe>
    with SingleTickerProviderStateMixin {
  late final AnimationController _controladorCielo;
  late final EstadoPistaPuzzle _pista;
  late ProblemaPorcentajeDe _problema;
  int? _indiceSeleccionado;
  bool _revelado = false;
  bool _mostrandoDemo = false;
  static const _idDemo = 'porcentaje_de';
  ...
  void initState() {
    ...
    _problema = widget.problemaPredeterminado ??
        GeneradorPorcentajeDe().generar(dificultad: 1);
  ...
  void _elegir(int indice) {
    ...
    if (_problema.esCorrecta(indice)) {
      ...
      comprobarYAyudarSiProcede(context, _pista, TipoFragmentoEnTejado.porcentajeDe);
      if (!mounted) return;
        ...
        builder: (_, __) {
          return Stack(
            ...
            children: [
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          ...
                          const Spacer(),
                          Text(AppLocalizations.of(contexto).puzzleHeaderQuePorcentaje,
                            style: const TextStyle(
                      ...
                      Text(
                        AppLocalizations.of(contexto).puzzleInstrQuePorcentaje,
                      ...
                      const SizedBox(height: 22),
                      _TarjetaPregunta(
                        parte: _problema.parte,
                        total: _problema.total,
                      ),
              ...
              BotonAyudaPuzzle(destacar: _pista.activa, tipo: TipoFragmentoEnTejado.porcentajeDe),
              if (_mostrandoDemo)
                OverlayDemoPuzzle(
...
class _TarjetaPregunta extends StatelessWidget {
  ...
  const _TarjetaPregunta({required this.parte, required this.total});
