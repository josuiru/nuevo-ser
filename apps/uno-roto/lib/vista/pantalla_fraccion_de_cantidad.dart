...
/// cantidad) pero con fracción.
class PantallaFraccionDeCantidad extends StatefulWidget {
  final ProblemaFraccionDeCantidad? problemaPredeterminado;

  const PantallaFraccionDeCantidad({
    super.key,
    this.problemaPredeterminado,
  });

  @override
  State<PantallaFraccionDeCantidad> createState() =>
      _PantallaFraccionDeCantidadState();
}

class _PantallaFraccionDeCantidadState
    extends State<PantallaFraccionDeCantidad>
    with SingleTickerProviderStateMixin {
  late final AnimationController _controladorCielo;
  late final EstadoPistaPuzzle _pista;
  late ProblemaFraccionDeCantidad _problema;
  int? _indiceSeleccionado;
  bool _revelado = false;
  bool _mostrandoDemo = false;
  static const _idDemo = 'fraccion_de_cantidad';
  ...
  void initState() {
    ...
    _problema = widget.problemaPredeterminado ??
        GeneradorFraccionDeCantidad().generar(dificultad: 1);
  ...
  void _elegir(int indice) {
    ...
    if (_problema.esCorrecta(indice)) {
      ...
      comprobarYAyudarSiProcede(context, _pista, TipoFragmentoEnTejado.fraccionDeCantidad);
      if (!mounted) return;
        ...
        builder: (_, __) {
          return Stack(
            ...
            children: [
              ...
              BotonAyudaPuzzle(destacar: _pista.activa, tipo: TipoFragmentoEnTejado.fraccionDeCantidad),
              if (_mostrandoDemo)
                OverlayDemoPuzzle(
