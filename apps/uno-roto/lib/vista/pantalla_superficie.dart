...
/// (×10 por peldaño) en lugar del factor de áreas (×100).
class PantallaSuperficie extends StatefulWidget {
  final ProblemaSuperficie? problemaPredeterminado;

  const PantallaSuperficie({super.key, this.problemaPredeterminado});

  @override
  State<PantallaSuperficie> createState() => _PantallaSuperficieState();
}

class _PantallaSuperficieState extends State<PantallaSuperficie>
    with SingleTickerProviderStateMixin {
  late final AnimationController _controladorCielo;
  late final EstadoPistaPuzzle _pista;
  late ProblemaSuperficie _problema;
  int? _indiceSeleccionado;
  bool _revelado = false;
  bool _mostrandoDemo = false;
  static const _idDemo = 'superficie';
  ...
  void initState() {
    ...
    _problema = widget.problemaPredeterminado ??
        GeneradorSuperficie().generar(dificultad: 1);
  ...
  void _elegir(int indice) {
    ...
    if (_problema.esCorrecta(indice)) {
      ...
      comprobarYAyudarSiProcede(context, _pista, TipoFragmentoEnTejado.superficie);
      if (!mounted) return;
        ...
        builder: (_, __) {
          return Stack(
            ...
            children: [
              ...
              BotonAyudaPuzzle(destacar: _pista.activa, tipo: TipoFragmentoEnTejado.superficie),
              if (_mostrandoDemo)
                OverlayDemoPuzzle(
