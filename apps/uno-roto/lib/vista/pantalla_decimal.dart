...
class PantallaDecimal extends StatefulWidget {
  ...
  final DecimalConocido? decimalObjetivo;

  const PantallaDecimal({super.key, this.decimalObjetivo});

  @override
  State<PantallaDecimal> createState() => _PantallaDecimalState();
}

class _PantallaDecimalState extends State<PantallaDecimal>
    with SingleTickerProviderStateMixin {
  late final AnimationController _controladorCielo;
  late final EstadoPistaPuzzle _pista;
  late ProblemaDecimal _problema;
  int? _indiceSeleccionado;
  bool _revelado = false;
  bool _mostrandoDemo = false;
  static const _idDemo = 'decimal';
  ...
  void initState() {
    ...
    final generador = GeneradorDecimal();
    _problema = widget.decimalObjetivo != null
        ? generador.generarDesde(widget.decimalObjetivo!)
        : generador.generar();
  ...
  void _elegir(int indice) {
    ...
    if (indice == _problema.indiceCorrecto) {
      ...
      comprobarYAyudarSiProcede(context, _pista, TipoFragmentoEnTejado.decimal);
      if (!mounted) return;
        ...
        builder: (_, __) {
          return Stack(
            ...
            children: [
              ...
              BotonAyudaPuzzle(destacar: _pista.activa, tipo: TipoFragmentoEnTejado.decimal),
              if (_mostrandoDemo)
                OverlayDemoPuzzle(
...
class _TarjetaFraccion extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext contexto) {
    const colorAcento = Color(0xFF7EE8D7);
    return Container(
      width: 160,
      height: 140,
      decoration: BoxDecoration(
