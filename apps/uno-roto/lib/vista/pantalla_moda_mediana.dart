...
class PantallaModaMediana extends StatefulWidget {
  ...
  const PantallaModaMediana({super.key, this.problemaPredeterminado});

  @override
  State<PantallaModaMediana> createState() => _PantallaModaMedianaState();
}

class _PantallaModaMedianaState extends State<PantallaModaMediana>
    with SingleTickerProviderStateMixin {
  late final AnimationController _controladorCielo;
  late final EstadoPistaPuzzle _pista;
  late ProblemaModaMediana _problema;
  int? _indiceSeleccionado;
  bool _revelado = false;
  bool _mostrandoDemo = false;
  static const _idDemo = 'moda_mediana';
  ...
  void initState() {
    ...
    _problema = widget.problemaPredeterminado ??
        GeneradorModaMediana().generar(dificultad: 1);
  ...
  void _elegir(int indice) {
    ...
    if (_problema.esCorrecta(indice)) {
      ...
      comprobarYAyudarSiProcede(context, _pista, TipoFragmentoEnTejado.modaMediana);
      if (!mounted) return;
        ...
        builder: (_, __) {
          return Stack(
            ...
            children: [
              ...
              BotonAyudaPuzzle(destacar: _pista.activa, tipo: TipoFragmentoEnTejado.modaMediana),
              if (_mostrandoDemo)
                OverlayDemoPuzzle(
