...
/// cuatro candidatos. La trampa estrella siempre presente: el cálculo
/// de izquierda a derecha sin respetar la prioridad de × y ÷.
class PantallaJerarquia extends StatefulWidget {
  final ProblemaJerarquia? problemaPredeterminado;

  const PantallaJerarquia({super.key, this.problemaPredeterminado});

  @override
  State<PantallaJerarquia> createState() => _PantallaJerarquiaState();
}

class _PantallaJerarquiaState extends State<PantallaJerarquia>
    with SingleTickerProviderStateMixin {
  late final AnimationController _controladorCielo;
  late final EstadoPistaPuzzle _pista;
  late ProblemaJerarquia _problema;
  int? _indiceSeleccionado;
  bool _revelado = false;
  bool _mostrandoDemo = false;
  static const _idDemo = 'jerarquia';
  ...
  void initState() {
    ...
    _problema = widget.problemaPredeterminado ??
        GeneradorJerarquia().generar(dificultad: 1);
  ...
  void _elegir(int indice) {
    ...
    if (_problema.esCorrecta(indice)) {
      ...
      comprobarYAyudarSiProcede(context, _pista, TipoFragmentoEnTejado.jerarquia);
      if (!mounted) return;
        ...
        builder: (_, __) {
          return Stack(
            ...
            children: [
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      _TarjetaExpresion(
                        expresion: formatearExpresion(_problema),
                      ),
              ...
              BotonAyudaPuzzle(destacar: _pista.activa, tipo: TipoFragmentoEnTejado.jerarquia),
              if (_mostrandoDemo)
                OverlayDemoPuzzle(
...
class _TarjetaExpresion extends StatelessWidget {
  ...
  const _TarjetaExpresion({required this.expresion});
