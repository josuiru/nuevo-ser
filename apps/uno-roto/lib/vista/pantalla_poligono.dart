...
/// reconocimiento — el primer puzzle del dominio GEO.
class PantallaPoligono extends StatefulWidget {
  final ProblemaPoligono? problemaPredeterminado;

  const PantallaPoligono({super.key, this.problemaPredeterminado});

  @override
  State<PantallaPoligono> createState() => _PantallaPoligonoState();
}

class _PantallaPoligonoState extends State<PantallaPoligono>
    with SingleTickerProviderStateMixin {
  late final AnimationController _controladorCielo;
  late final EstadoPistaPuzzle _pista;
  late ProblemaPoligono _problema;
  int? _indiceSeleccionado;
  bool _revelado = false;
  bool _mostrandoDemo = false;
  static const _idDemo = 'poligono';
  ...
  void initState() {
    ...
    _problema = widget.problemaPredeterminado ??
        GeneradorPoligono().generar(dificultad: 1);
  ...
  void _elegir(int indice) {
    ...
    if (_problema.esCorrecta(indice)) {
      ...
      comprobarYAyudarSiProcede(context, _pista, TipoFragmentoEnTejado.poligono);
      if (!mounted) return;
        ...
        builder: (_, __) {
          return Stack(
            ...
            children: [
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 22),
                      _TarjetaPoligono(numeroDeLados: _problema.numeroDeLados),
              ...
              BotonAyudaPuzzle(destacar: _pista.activa, tipo: TipoFragmentoEnTejado.poligono),
              if (_mostrandoDemo)
                OverlayDemoPuzzle(
...
/// Caja con el polígono dibujado.
class _TarjetaPoligono extends StatelessWidget {
  final int numeroDeLados;
  const _TarjetaPoligono({required this.numeroDeLados});
  ...
  Widget build(BuildContext contexto) {
    return Container(
      ...
      child: SizedBox(
        ...
        height: 160,
        child: CustomPaint(
          painter: _PintorPoligono(numeroDeLados: numeroDeLados),
        ),
      ),
    );
  }
...
class _PintorPoligono extends CustomPainter {
  final int numeroDeLados;
  _PintorPoligono({required this.numeroDeLados});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final ruta = Path();
    // Empezamos por el vértice superior (rotación de -π/2).
    for (var i = 0; i < numeroDeLados; i++) {
      final angulo = -math.pi / 2 + 2 * math.pi * i / numeroDeLados;
      final punto = centro +
          Offset(math.cos(angulo) * radio, math.sin(angulo) * radio);
      if (i == 0) {
        ruta.moveTo(punto.dx, punto.dy);
      } else {
  ...
  @override
  bool shouldRepaint(covariant _PintorPoligono oldDelegate) =>
      oldDelegate.numeroDeLados != numeroDeLados;
}
