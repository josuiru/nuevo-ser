...
class PantallaPorcentaje extends StatefulWidget {
  final PorcentajeConocido? porcentajeObjetivo;

  const PantallaPorcentaje({super.key, this.porcentajeObjetivo});

  @override
  State<PantallaPorcentaje> createState() => _PantallaPorcentajeState();
}

class _PantallaPorcentajeState extends State<PantallaPorcentaje>
    with SingleTickerProviderStateMixin {
  late final AnimationController _controladorCielo;
  late final EstadoPistaPuzzle _pista;
  late ProblemaPorcentaje _problema;
  int? _indiceSeleccionado;
  bool _revelado = false;
  bool _mostrandoDemo = false;
  static const _idDemo = 'porcentaje';
  ...
  void initState() {
    ...
    final generador = GeneradorPorcentaje();
    _problema = widget.porcentajeObjetivo != null
        ? generador.generarDesde(widget.porcentajeObjetivo!)
        : generador.generar();
  ...
  void _elegir(int indice) {
    ...
    if (indice == _problema.indiceCorrecto) {
      ...
      comprobarYAyudarSiProcede(context, _pista, TipoFragmentoEnTejado.porcentaje);
      if (!mounted) return;
        ...
        builder: (_, __) {
          return Stack(
            ...
            children: [
                ...
                child: Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      _TarjetaPorcentaje(
                          etiqueta: _problema.etiquetaPorcentaje),
              ...
              BotonAyudaPuzzle(destacar: _pista.activa, tipo: TipoFragmentoEnTejado.porcentaje),
              if (_mostrandoDemo)
                OverlayDemoPuzzle(
...
class _TarjetaPorcentaje extends StatelessWidget {
  ...
  const _TarjetaPorcentaje({required this.etiqueta});

  @override
  Widget build(BuildContext contexto) {
    const colorAcento = Color(0xFFFF7ED0);
    return Container(
      width: 160,
      height: 140,
      decoration: BoxDecoration(
