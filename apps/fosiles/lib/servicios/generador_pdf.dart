...
pw.Widget _bloqueHallazgo(Hallazgo h) {
  ...
  return pw.Container(
    ...
    child: pw.Column(
      ...
      children: [
        pw.Row(children: [
          pw.Expanded(child: pw.Text(h.especie.isEmpty ? '(sin nombre)' : h.especie, style: pw.TextStyle(fontWeight: pw.FontWeight.bold))),
          pw.Text(_formatearFecha(fecha), style: const pw.TextStyle(fontSize: 10)),
        ]),
        if (h.edad.isNotEmpty) pw.Text('Edad: ${h.edad}', style: const pw.TextStyle(fontSize: 10)),
        if (h.formacion.isNotEmpty) pw.Text('Formación: ${h.formacion}', style: const pw.TextStyle(fontSize: 10)),
