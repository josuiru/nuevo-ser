...
- **B11 (parcial)** — `dominio/fenologia.dart` extendido con `estacionesEnTransicion()` (margen ±15 días sobre cortes astronómicos) y `NotasFenologicasIberia.para()` con notas hardcoded para ES-NA-PA, ES-BI, ES-MD + fallback ES. Marcado pendiente de calendario curado por ornitólogos/botánicos.
...
Tests: 232 verde en el-cuaderno (Dart) + 51 en `nuevo_ser_companion` + smoke PHP verde + paridad P5 12/12 + paridad calibración Brier verde. APK debug compila.

**Bloque B — pendiente de decisiones humanas**, no bloquea piloto interno (familias del operador) pero sí piloto público:
B1 validación científica catálogo · B2 traducciones eu/ca (parcial: strings principales hechos como fallback de experto) · B3 LOPDGDD + política privacidad real · B4 ilustradora botánica · B5 `geolocator` + `flutter_map` + MBTiles (parcial: geolocator cableado, pantallas no lo consumen) · B6 UX rica del dibujo (parcial: AnchoTrazo + undo) · B7 auth profesor + vista aula (cliente + UI hechos como fallback; policy escolar pendiente) · B8 asesoría psicológica caso 1 doc 15 · B9 WCAG 2.1 AA + paquete `pdf` (parcial: pdf cableado) · B10 captación 12-15 familias · B11 calendario fenológico curado (parcial: ±15d + notas Iberia hardcoded) · B12 firma release Android + canal distribución.
