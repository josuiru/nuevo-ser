...
| F1-A9 Libro ingresos/gastos | **✅ parcial (esta sesión, modelos + BD + tests)** | 4 modelos POJO `Tercero`, `ConfiguracionFiscal`, `ApunteIngreso` (con tipos venta_aceituna / venta_aceite_envasado / venta_aceite_granel / alquiler_terreno / ayuda_pac / subvencion_autonomica / subproducto_alperujo / otro y campos `parcelaId`, `loteAceiteId`, `variedadId` para trazabilidad), `ApunteGasto` (con tipos olivar: insumos_olivar / fitosanitarios / recoleccion / molturacion_externa / envasado / analiticas / cuota_dop / maquinaria / mano_obra / combustible / seguros / transporte / certificacion / otro + `imputacion` parcela/variedad/general + FK opcional a `tratamientoId` para sinergia con libro PAC + helper `esGasoilAgricola`). Migración BD v1→v2 puramente aditiva (4 tablas con FK + 10 índices). Reglas IVA olivar en `ConfiguracionFiscal`: aceituna 4% (general) o 12% compensación REAGP, aceite envasado 4% (alimento básico, fuera de REAGP), aceite a granel 4% (sujeto a confirmación), alquiler agrícola exento. CRUD básico en `BaseDatosSoleraAceitera` (insertar/listar terceros, configuración, ingresos y gastos con filtro temporal). 17 tests POJO (`test/economico_test.dart`) cubren round-trip, reglas IVA REAGP, imputaciones y helpers derivados. Tercer guardrail PROVISIONAL en `sello_provisional_test.dart` custodia las reglas fiscales. UI (pantalla configuración + terceros + libro económico + formularios + extracto PDF) pendiente para F1-A9b/F2 |
