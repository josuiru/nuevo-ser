...
/// Empaqueta la BD (`solera_viticultura.db`) y la carpeta de fotos
...
/// un directorio temporal, valida que tenga `solera_viticultura.db`
/// dentro y sustituye los archivos. Antes de sobrescribir, hace **un
/// backup automático del estado actual** por si la restauración va mal.
class BackupServicio {
  static const _claveUltimoBackupMs = 'solera_viticultura.backup.ultimo_ms';
  static const _nombreBd = 'solera_viticultura.db';
  ...
  static Future<File> crearZip() async {
    ...
    final destino = File(path_lib.join(tmp.path, 'solera-viticultura-backup-$marcaTemporal.zip'));
    ...
    await BaseDatosSoleraViticultura.instancia.cerrar();
  ...
  /// si la restauración deja la app inconsistente. Devuelve el fichero
  /// del backup pre-restore para que el usuario lo conserve si lo desea.
  static Future<File> restaurarZip(File zipOrigen) async {
    ...
    // Validación previa: leer el zip y comprobar que tiene la BD.
    final bytes = await zipOrigen.readAsBytes();
    final archive = ZipDecoder().decodeBytes(bytes);
    final tieneBd = archive.any((f) => f.name == _nombreBd);
    if (!tieneBd) {
      throw Exception(
        'El zip no contiene $_nombreBd — no parece un backup de Solera Viticultura.',
      );
    }

    // Backup pre-restore por seguridad.
    final preRestore = await crearZip();

    // Cerrar BD y descomprimir.
    await BaseDatosSoleraViticultura.instancia.cerrar();
    final docs = await getApplicationDocumentsDirectory();
    // Borrar la BD existente (incluyendo wal/shm) para que no quede
    // estado inconsistente.
    for (final nombre in [_nombreBd, '$_nombreBd-wal', '$_nombreBd-shm']) {
      final f = File(path_lib.join(docs.path, nombre));
      if (await f.exists()) await f.delete();
    }
    // No borramos las fotos antiguas — el zip las trae, pero si el
    // usuario tiene algo extra fuera del backup queremos conservarlo.
    // Las fotos del zip se sobrescriben encima.
  ...
  /// backup. Default 7 días. La pantalla principal puede usar esto
  /// para sugerir hacer backup, pero **nunca** ejecutarlo
  ...
  static Future<Database> reabrirBaseDatos() => BaseDatosSoleraViticultura.instancia.basedatos;
}
